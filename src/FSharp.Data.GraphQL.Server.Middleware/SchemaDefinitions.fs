...
module SchemaDefinitions =
    ...
    let rec private coerceObjectListFilterInput x : ObjectListFilter GQLResult =
        ...
        let rec mapFilter (name : string, value : InputValue) =
            let mapFilters fields =
                let coerceResults = fields |> Seq.map coerceObjectListFilterInput
                match coerceResults with
                | ArrayErrors errs -> Error errs
                | ArrayValues coerced -> coerced |> removeNoFilter |> Seq.toList |> Ok
        ...
        and mapInput value : ObjectListFilter GQLResult =
            let filterResults = value |> Map.toSeq |> Seq.map mapFilter
    ...
    let private coerceObjectListFilterValue (x : obj) : ObjectListFilter GQLResult =
        match x with
