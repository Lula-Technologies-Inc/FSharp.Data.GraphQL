...
and FieldExecuteMap(compiler : FieldExecuteCompiler) =
    ...
    member _.GetExecute(typeName: string, fieldName: string) =
        ...
        | true, value -> fst value
        | false, _ -> Unchecked.defaultof<ExecuteField>
    ...
    member _.GetArgs(typeName : string, fieldName : string) =
        let key = getKey typeName fieldName
        match map.TryGetValue key with
        | true, value -> snd value
        | false, _ -> Unchecked.defaultof<InputFieldDef []>
...
and ExecutionContext =
    { /// GraphQL schema definition.
      ...
      Errors : ConcurrentBag<GQLProblemDetails>
...
and ResolveFieldContext =
    ...
    member x.AddError error = x.Context.Errors.Add (GQLProblemDetails.OfFieldError x.Path error)

    /// Tries to find an argument by provided name.
    member x.TryArg(name : string) : 't option =
        match HashMap.vTryFind name x.Args with
        | ValueSome o -> Some(o :?> 't) // TODO: Use Convert.ChangeType
        | ValueNone -> None

    /// Returns an argument by provided name. If argument was not found
    /// and exception will be thrown.
    member x.Arg(name : string) : 't GQLResult =
        match HashMap.vTryFind name x.Args with
        | ValueSome found -> downcast found |> Ok
        | ValueNone ->
            Error [{ new IGQLError with member _.Message = $"Argument '%s{name}' was not provided within context of a field '%s{x.ExecutionInfo.Identifier}'. Check if it was supplied within GraphQL query." }]

/// Function type for the compiled field executor.
and ExecuteField = ResolveFieldContext -> obj -> AsyncVal<GQLResult>
...
and [<CustomEquality; NoComparison>] ScalarDefinition<'Val> =
    ...
    interface ScalarDef with
        ...
        member x.CoerceOutput value = (x.CoerceOutput value) |> Result.map box
...
and internal NullableDefinition<'Val> =
    ...
    interface TypeDef with
        member _.Type = typeof<GQLResult<'Val>>
...
and ExecuteInput = InputValue -> ImmutableDictionary<string, obj> -> GQLResult
...
module Resolve =
    ...
    let private boxifyExpr expr : ResolveFieldContext -> obj -> GQLResult =
    ...
    let private boxifyExprAsync expr : ResolveFieldContext -> obj -> Async<GQLResult> =
        match unwrapExpr expr with
